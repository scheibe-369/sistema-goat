...
export function LeadFilters({ isOpen, onClose, filters, onFiltersChange }: LeadFiltersProps) {
  ...
  const groupOptions = ["Clientes GOAT", "Networking"];
          ...
          <div className="flex items-center justify-between p-6 border-b border-goat-gray-700">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-goat-gray-400 text-sm">Filtre os leads conforme necessário</p>
              </div>
            </div>
            <Button
              onClick={onClose}
                ...
                <div className="space-y-3">
                  {groupOptions.map((group) => (
                    <div key={group} className="flex items-center space-x-3">
                      <Checkbox
                        id={`group-${group}`}
                        checked={localFilters.groups.includes(group)}
                        onCheckedChange={(checked) => handleGroupChange(group, checked as boolean)}
                        className="border-goat-gray-600 data-[state=checked]:bg-goat-purple data-[state=checked]:border-goat-purple"
                      />
                      <Label 
                        htmlFor={`group-${group}`} 
