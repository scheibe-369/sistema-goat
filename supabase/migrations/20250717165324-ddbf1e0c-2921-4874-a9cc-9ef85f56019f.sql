...
-- Corrigir definitivamente a função para usar os nomes corretos das colunas
...
BEGIN
  ...
  -- Inserir mensagem na tabela messages usando apenas campos existentes
...
-- Também vamos verificar se a função webhook_insert_message está correta
...
DECLARE
  ...
  user_uuid UUID;
BEGIN
  -- Extrair user_id do webhook_data
  user_uuid := (webhook_data->>'user_id')::UUID;
  
  -- Se não houver user_id, tentar usar auth.uid()
  IF user_uuid IS NULL THEN
    SELECT auth.uid() INTO user_uuid;
  END IF;
  
  -- Se ainda não houver usuário, retornar erro
  IF user_uuid IS NULL THEN
    RAISE EXCEPTION 'user_id obrigatório nos dados do webhook ou usuário deve estar autenticado';
  END IF;
  
  -- Chamar a função principal com os campos corretos
  SELECT public.process_webhook_message(
    user_uuid,
    webhook_data->>'numero',
    webhook_data->>'mensagem',
    COALESCE((webhook_data->>'direcao')::BOOLEAN, false),
    COALESCE((webhook_data->>'data_hora')::TIMESTAMP WITH TIME ZONE, NOW()),
    COALESCE(webhook_data->>'conversa_id', webhook_data->>'numero'),
...
BEGIN
  ...
  RAISE NOTICE 'Teste direto funcionou. ID da mensagem: %', test_message_id;
  ...
  test_webhook_data := jsonb_build_object(
    ...
    'direcao', false,
    'data_hora', NOW(),
    'conversa_id', 'webhook_test_' || extract(epoch from now()),
    'nome_contato', 'Webhook Test'
  );
  
  SELECT public.webhook_insert_message(test_webhook_data) INTO test_message_id;
  
  RAISE NOTICE 'Teste webhook funcionou. ID da mensagem: %', test_message_id;
END;
$$;
