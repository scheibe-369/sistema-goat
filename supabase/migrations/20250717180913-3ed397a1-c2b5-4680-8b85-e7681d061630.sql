...
-- Ajustar políticas da tabela messages para permitir inserção sem autenticação
...
-- Manter a política de visualização baseada em auth para o frontend
...
-- Ajustar políticas da tabela conversations para permitir criação sem autenticação
...
BEGIN
  ...
  SELECT public.process_webhook_message(
    ...
    false,
    NOW(),
    'msg_auto_' || extract(epoch from now()),
    'João Silva Teste'
  ) INTO test_message_id;
  
  RAISE NOTICE 'Teste completo executado. ID da mensagem: %', test_message_id;
END;
$$;