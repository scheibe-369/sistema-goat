-- Corrigir a função process_webhook_message para definir valor do lead como null
...
BEGIN
  ...
  -- Buscar o nome do estágio padrão "Sem atendimento" para o usuário
  SELECT name INTO default_stage_name
  FROM public.stages
  WHERE user_id = p_user_id AND name = 'Sem atendimento'
  LIMIT 1;
  
  -- Se não encontrar, usar um fallback
  IF default_stage_name IS NULL THEN
    default_stage_name := 'Sem atendimento';
  END IF;
  ...
  IF contato_uuid IS NULL AND NOT p_direcao THEN
    ...
    -- Se não existir lead, criar automaticamente com etapa "Sem atendimento" e valor null
    ...
    END IF;
  ELSIF contato_uuid IS NOT NULL THEN
