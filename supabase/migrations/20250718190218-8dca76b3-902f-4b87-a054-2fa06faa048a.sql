-- Remove função duplicada sem p_media_key
DROP FUNCTION IF EXISTS public.process_webhook_message(uuid, text, text, boolean, timestamp with time zone, text, text, text, text, bigint);

-- Garante que só existe uma versão da função (com p_media_key)
...
BEGIN
  ...
  -- PASSO 3: Inserir mensagem incluindo todos os campos de mídia
